    ...
    <section id="troubleshooter" className="py-12 md:py-20 border-t border-[#2A2A3A]/30">
      <div className="max-w-3xl mx-auto px-4 md:px-8">
        ...
        <div className="mb-8 md:mb-12">
          ...
          <h2 className="text-2xl md:text-3xl font-bold tracking-tight uppercase mt-2">
            Diagnostic Troubleshooter
          </h2>
          <p className="text-sm text-[#7A7A90] mt-2 max-w-xl">
            Answer three quick questions about your car&apos;s behavior and get prioritized setup
            adjustments ranked from easiest to most involved.
          </p>
        </div>
          ...
          {result ? (
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <button
                  onClick={handleBack}
                  className="text-sm text-[#7A7A90] hover:text-[#D4D4E0] flex items-center gap-1 min-h-[48px]"
                >
                ...
                <button onClick={handleReset} className="text-sm text-[#00B4FF] min-h-[48px]">
                  Start Over
                </button>
              </div>

              <div>
                <h3 className="text-2xl font-bold tracking-tight uppercase">{result.title}</h3>
              ...
              <div>
                <h4 className="text-xs font-semibold text-[#7A7A90] uppercase tracking-wider mb-4">
                  ...
                  <span className="text-[#555570] normal-case font-normal ml-2">
                ...
                <div className="space-y-3">
                  {result.adjustments.map((adj, i) => (
                    <div
                      key={i}
                      className="bg-[#0A0A0F] border border-[#2A2A3A]/50 rounded-lg p-4"
                    >
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-1">
                            <span className="text-[10px] px-2 py-0.5 rounded-full bg-[#2A2A3A] text-[#7A7A90] uppercase">
                              {adj.category}
                            </span>
                          </div>
                          <p className="font-semibold text-sm">{adj.action}</p>
                          <p className="text-xs text-[#00B4FF] font-mono mt-1">{adj.amount}</p>
                          <p className="text-xs text-[#7A7A90] mt-2 leading-relaxed">
                            {adj.explanation}
                          </p>
                        </div>
                      </div>
              ...
              {/* CTA banner */}
              <div className="mt-6 rounded-lg border border-[#2A2A3A]/50 bg-[#0A0A0F] p-4 text-center">
                <p className="text-sm text-[#7A7A90]">
                  Sign up free to log fixes and track what works.
                </p>
                <Link
                  href="/sign-up"
                  className="inline-block mt-2 text-sm font-semibold text-[#00B4FF] hover:underline"
                >
                  Create Account →
                </Link>
              </div>
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm text-[#555570] font-mono">
