  ...
  return (
    <div className="space-y-3">
      ...
      <div className="bg-[#252525] rounded-md p-3">
        <p className="text-[10px] text-[#666] uppercase mb-2">Build Summary</p>
        <div className="grid grid-cols-2 gap-2 text-xs">
          <div className="flex justify-between">
            <span className="text-[#888]">Heads</span>
            <span className="font-mono">{head?.name || result.config.headId}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-[#888]">Cam</span>
            <span className="font-mono">{cam?.lift}" / {cam?.duration}°</span>
          </div>
          <div className="flex justify-between">
            <span className="text-[#888]">Bore</span>
            <span className="font-mono">{result.config.bore.toFixed(3)}"</span>
          </div>
          <div className="flex justify-between">
            <span className="text-[#888]">Piston</span>
...
function StatCard({ label, value, unit, subValue, accent, color }: {
  ...
  return (
    <div className={`bg-[#252525] rounded-md p-3 ${accent ? 'ring-1 ring-[#FFD600]/15' : ''}`}>
      ...
      {subValue && <p className="text-[10px] text-[#666] mt-0.5">{subValue}</p>}
    </div>
  )
}
