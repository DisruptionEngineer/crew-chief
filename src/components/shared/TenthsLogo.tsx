...
export function TenthsLogo({ size = 40, className = '' }: { size?: number; className?: string }) {
  const id = useId()
  const carbonId = `carbonWeave-${id}`
  const glowId = `logoGlow-${id}`
  const metalId = `metalEdge-${id}`
  ...
  return (
    ...
    >
      <defs>
        ...
        <pattern id={carbonId} x="0" y="0" width="4" height="4" patternUnits="userSpaceOnUse">
        ...
        {/* Brushed metal edge */}
        <linearGradient id={metalId} x1="0" y1="0" x2="48" y2="48" gradientUnits="userSpaceOnUse">
      ...
      <rect x="1" y="1" width="46" height="46" rx="10" fill={`url(#${metalId})`} />

      {/* Inner body — deep carbon */}
      <rect x="2.5" y="2.5" width="43" height="43" rx="8.5" fill="#0F0F1A" />

      {/* Carbon texture overlay */}
      <rect x="2.5" y="2.5" width="43" height="43" rx="8.5" fill={`url(#${carbonId})`} />

      {/* Subtle glow wash */}
      <rect x="2.5" y="2.5" width="43" height="43" rx="8.5" fill={`url(#${glowId})`} />
