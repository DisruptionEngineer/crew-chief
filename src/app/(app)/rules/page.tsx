    ...
    <div className="space-y-6">
      ...
      <div className="animate-fade-up">
        ...
        <div className="flex items-center gap-2 mt-1">
          <svg className="w-3.5 h-3.5 text-[#FF8A00]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round">
            <path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z" />
            <circle cx="12" cy="10" r="3" />
          </svg>
          <p className="text-sm text-[#888]">{activeDivision.trackName}</p>
          <span className="text-[10px] text-[#555]">2026 Season</span>
        </div>
      </div>
      ...
      <div className="bg-[#1A1A1A] border border-[#333] rounded-lg p-4">
        <p className="text-sm text-[#F5F5F5] leading-relaxed">{activeDivision.description}</p>
        <div className="flex flex-wrap gap-3 mt-3 text-[11px] text-[#888]">
          <span>Min weight: <span className="text-[#F5F5F5] font-mono">{activeDivision.minWeightLbs} lbs</span></span>
          {activeDivision.minLeftPct && (
            <span>Min left: <span className="text-[#F5F5F5] font-mono">{activeDivision.minLeftPct}%</span></span>
          )}
        ...
        {activeDivision.notes && (
          <p className="text-[10px] text-[#666] mt-2 leading-relaxed">{activeDivision.notes}</p>
        )}
      </div>
      ...
      {totalChecks > 0 && (
        ...
        >
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs font-mono text-[#888]">{checkedCount}/{totalChecks}</span>
          </div>
          {/* Progress bar */}
      ...
      {showTechChecklist && (
        <div className="space-y-4">
          {Object.entries(techCheckCategories).map(([category, items]) => (
            <div key={category} className="bg-[#1A1A1A] border border-[#333] rounded-lg p-4">
              <h3 className="text-xs font-semibold text-[#888] uppercase tracking-wider mb-3">{category}</h3>
                ...
                {items.map(item => (
                  <label key={item.id} className="flex items-start gap-3 cursor-pointer min-h-[44px] py-1">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-[#555] mt-0.5">Rule {item.rule}</p>
                    </div>
                  </label>
                ))}
              </div>
            ...
            return (
              <div key={cat} className={`bg-[#1A1A1A] border rounded-lg overflow-hidden transition-all duration-200 ${
                ...
                >
                  ...
                  <svg className={`w-4 h-4 text-[#666] transition-transform duration-200 ${isExpanded ? 'rotate-180' : ''}`} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}>
                  ...
                  <div className="px-4 pb-3">
                    <div className="flex flex-wrap gap-2">
                      {catRules.slice(0, 3).map(rule => (
                        <span key={rule.id} className="text-[10px] text-[#888] bg-[#252525] px-2 py-1 rounded">
