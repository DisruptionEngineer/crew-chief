    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <button onClick={handleBack} className="text-sm text-[#7A7A90] hover:text-[#D4D4E0] flex items-center gap-1 min-h-[48px]">
            <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><polyline points="15 18 9 12 15 6" /></svg>
            Back
          </button>
          <button onClick={handleReset} className="text-sm text-[#00B4FF] min-h-[48px]">Start Over</button>
        </div>
        ...
        <div>
          <h2 className="text-xs font-semibold text-[#7A7A90] uppercase tracking-wider mb-4">
            Try These Adjustments
            <span className="text-[#555570] normal-case font-normal ml-2">(easiest → most involved)</span>
          </h2>

          <div className="space-y-3">
            {result.adjustments.map((adj, i) => (
              <div key={i} className="bg-[#14141F] border border-[#2A2A3A] rounded-lg p-4">
      ...
      <div className="space-y-3">
        {currentStep.options.map((option, i) => (
          <button
            ...
            className="w-full text-left bg-[#14141F] border border-[#2A2A3A] rounded-lg p-4 hover:border-[#00B4FF]/30 transition-colors min-h-[72px]"
          >
